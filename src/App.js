        ...
        const allTasks = this.props.allTasks.map((e) => {
            return (<div className='newTask' style={e.isComplete !== true ? { 'box-shadow': 'inset 0 0 10px #8c2222' } : { 'box-shadow': 'inset 0 0 10px #1d8520' }}>
                ...
                <div className='taskButtons'>
                    <button id='completeButton' className='taskButton' onClick={() => { this.handleComplete(e.id) }}>Complete</button>
                    <button id='deleteButton' className='taskButton' onClick={() => { this.handleDelete(e.id) }}>Delete</button>
                </div>
            </div>)
        })
        return (
